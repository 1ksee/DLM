...
% Meshed case with voltages: 6,22,67,132 kV
% CaseOrig = case_ieee30; PV_idx = [2:5]; Wind_idx = 6;
% Change transformers' ratio:
% CaseOrig.branch([11,12,15,36],9) = 0.99;

% Case with 20022 buses made from case141.m by duplicating full system in
% each node and connecting via transforms. PL, QL are decreased 31 times
structA = load('case20022_pfConv.mat'); 
CaseOrig = structA.caseA;

% B. ADDITIONAL (UNAVAILABLE) TEST CASES
% CaseOrig = Akirkeby_10kV_system;
% CaseOrig = case33_100MVA; PV_idx = [2:7,9,10]; Wind_idx = 8;
% CaseOrig = case33_200600Invs_100MVA; PV_idx = [2:7,9,10]; Wind_idx = 8;
% CaseOrig = case13659pegase;
% CaseOrig = case33_200600Invs_5DG_100MVA; PV_idx = [2:5]; Wind_idx = 6;
% CaseOrig = case16_200600Invs_100MVA;

% Case with 19741 buses made from case141.m by duplicating full system in
% each node and stitching. PL, QL are decreased 40 times
% structA = load('case19741.mat'); 
% CaseOrig = structA.caseA;

% "Radialized" case_ieee30
% CaseOrig = case_ieee30_radialized; PV_idx = [2:5]; Wind_idx = 6;
...
Niter = 10; % If Niter>1, then set TV=1
...
Period = 25; % 1 - for test purpose; 12 - means 12 hours; 24 - 03:00-19:30 at the first day in Garima's forecasting;
...
%         % Make a histogram plot for a 19k-bus system with original loading
%         figure(101)
%         histogram(case141.bus(:,3))
%         xlabel('Active load range, [kW]')
...
disp(['G. Case with Hybrid LLMA. MEAN: ',num2str(mean(TotdP_HybridLLMA_kW_List)),'; STD: ',num2str(std(TotdP_HybridLLMA_kW_List))])
disp(['H. Case with Hybrid LFMA. MEAN: ',num2str(mean(TotdP_HybridLFMA_kW_List)),'; STD: ',num2str(std(TotdP_HybridLFMA_kW_List))])
fprintf('\n') 
...
disp(['G. Case with Hybrid LLMA: ',num2str(size(unique(TotdP_HybridLLMA_kW_List),1))])
disp(['H. Case with Hybrid LFMA: ',num2str(size(unique(TotdP_HybridLFMA_kW_List),1))])
fprintf('\n') 
...
disp(['I. Hybrid LLMA: ',num2str(NInf_HybridLLMA)])
disp(['J. Hybrid LFMA: ',num2str(NInf_HybridLFMA)])
fprintf('\n') 
...
disp(['G. Case with Hybrid LLMA. MWH: ',...
    num2str(EnergyLosses_HybridLLMA_MWh),'; MONETARY: ',num2str(EnergyLosses_HybridLLMA_Money),'; Savings MONETARY: ',...
    num2str(EnergyLosses_base_Money-EnergyLosses_HybridLLMA_Money)])
disp(['H. Case with Hybrid LFMA. MWH: ',...
    num2str(EnergyLosses_HybridLFMA_MWh),'; MONETARY: ',num2str(EnergyLosses_HybridLFMA_Money),'; Savings MONETARY: ',...
    num2str(EnergyLosses_base_Money-EnergyLosses_HybridLFMA_Money)])
fprintf('\n') 

disp('9. HYBRID LLMA')
disp(['A. Number of inverters with non-zero Qg reserve in Hybrid LLMA. MEAN: ',num2str(mean(NNonZerdQLLMA_List)),'; STD: ',num2str(std(NNonZerdQLLMA_List))])
if SelectNCenInv == 1
    disp(['B. User has selected to have ',num2str(NCenInv_user),' centrally controlled inverters.'])
else
    disp(['B. Number of centrally controlled inverters in Hybrid LLMA.  MEAN: ',num2str(mean(NNonZerdQLLMA_List)),'; STD: ',num2str(std(NNonZerdQLLMA_List))])
end
% for i = 1:size(CenInv_Iter_array_tab_LLMA,2)
%     Gen_ID = mode(CenInv_Iter_array_tab_LLMA(:,i));
%     disp(['C. In Hybrid LLMA, the most frequent inverter #',num2str(i),' is in node ',num2str(Gen_ID)])
% end
fprintf('\n') 

disp('10. HYBRID LFMA')
disp(['A. Number of inverters with non-zero Qg reserve in Hybrid LFMA. MEAN: ',num2str(mean(NNonZerdQLFMA_List)),'; STD: ',num2str(std(NNonZerdQLFMA_List))])
if SelectNCenInv == 1
    disp(['B. User has selected to have ',num2str(NCenInv_user),' centrally controlled inverters.'])
else
    disp(['B. Number of centrally controlled inverters in Hybrid LFMA.  MEAN: ',num2str(mean(NNonZerdQLFMA_List)),'; STD: ',num2str(std(NNonZerdQLFMA_List))])
end
% for i = 1:size(CenInv_Iter_array_tab_LFMA,2)
%     Gen_ID = mode(CenInv_Iter_array_tab_LFMA(:,i));
%     disp(['C. In Hybrid LFMA, the most frequent inverter #',num2str(i),' is in node ',num2str(Gen_ID)])
% end


%% Making plots
if Plotting == 1
    % boxplot for comparison of random PV placement
    if Niter > 1 
        figure(5)
        boxplot1 = boxplot([dLos_Tur_base_List, dLos_NoCom_base_List],'Labels',{'Heuristic approach [3]','Proposed algorithm'});

        ylim([0 100])
        set(gca,'YTick',0:10:100)
        set(gca,'Ydir','reverse')
        ylabel('Loss decrease compared to the ''no-actions'' strategy (%)')
        % Import to tik-z (for further modification in Overleaf)
        if BP == 1 
            matlab2tikz('boxplot_90Send_1000iter_PgCoef08_CosFiLim08_QCoef0_Akirkeby.tex')
        end

        figure(10)
        size_box = size(TotdP_OPF_kW_List,1);
        boxplot10 = boxplot([TotdP_base_kW_List(1:size_box),...
        TotdP_Tur_kW_List(1:size_box), TotdP_NoCom_kW_List(1:size_box),...
        TotdP_OPF_kW_List],...
        'Labels',{'No-action strategy', 'Load measuring algorithm', 'Flow measuring algorithm', 'Centralized OPF'});
        ylabel('Active power losses (kW)')

    end

    if TV == 1
        % plot of total active PV generation and load
        figure(2)
        plot(TotPG_List) % too much variation
        hold on 
        plot(TotPL_List)
        hold off
        legend('Solar generation','Active power demand')

        if Period == 12
            title('Five minutes-based total solar generation and demand of Akirkeby system on June 16 at 04:00 - 12:00')
        end

        if Period == 365
            title('Five minutes-based total solar generation and demand of Akirkeby system in 2019')
        end

        xlabel('Time [5 min]')
        ylabel('Solar and demand [kW]')
    %     matlab2tikz('plot_TotPG_TotPL_Akirkeby_45PV_TimVar_Full2019.tex')

        % plot of total Qg: Turitsyn and our algorithm
        figure(3)
        plot(TotQG_ExclSlack_base_List,'LineWidth',2)
        hold on
        plot(TotQG_ExclSlack_Tur_List,'LineWidth',2) % too much variation
        hold on 
        plot(TotQG_ExclSlack_NoCom_List,'LineWidth',2)
        hold on
        plot(TotQG_ExclSlack_OPF_List,'LineWidth',2)
        hold off
        set(gca,'FontSize',30)
    %     legend('No-actions strategy','Heuristic approach','Proposed algorithm')
        legend('No-action strategy','Local load measuring algorithm','Local flow measuring algorithm','Centralized OPF')

    %     if Period == 12
    %         title('Five minutes-based total reactive power generation in Akirkeby system on June 16 at 04:00 - 12:00')
    %     end
        step = 12.5;
        xticks([step*0+step/2 step*1+step/2 step*2+step/2 step*3+step/2 step*4+step/2 step*5+step/2 ...
            step*6+step/2 step*7+step/2 step*8+step/2])
        xticklabels({'04:00','05:00','06:00','07:00','08:00','09:00','10:00','11:00','12:00'})
        axis([2 100 0 0.8])


        if Period == 365
            title('Five minutes-based total reactive power generation Akirkeby system in 2019')
        end

    %     xlabel('HH:MM')
    %     ylabel('Total reactive generation [MVAr]')
        ylh = ylabel('Total reactive generation [MVAr]');
        ylh.Position(1) = ylh.Position(1) - 1.5;  % move the label 0.1 data-units further down

        % plot of total active power losses
        figure(4)
        plot(TotdP_base_kW_List) % too much variation
        hold on 
        plot(TotdP_Tur_kW_List)
        hold on 
        plot(TotdP_NoCom_kW_List)    
        hold off
    %     legend('No-actions strategy','Heuristic approach','Proposed algorithm')
        legend('No-actions strategy','Load-based','Load- and flow-based')

    %     if Period == 12
    %         title('Five minutes-based total active power losses of Akirkeby system on June 16 at 04:00 - 12:00')
    %     end

        if Period == 365
            title('Five minutes-based total active power losses of Akirkeby system in 2019')
        end

        xlabel('Time [5 min]')
        ylabel('Total active power losses [kW]')

    end
end
%% Saving all variables
% cd 'C:\Users\ilgiz\Documents\ID EDGe\5 DGs Matpower simulations\';
% save RTDS_33bus_5DGs_k06_Sc1_FctY.mat
